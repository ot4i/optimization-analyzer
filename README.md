...
2. On Windows, open an IBM App Connect Enterprise Command Console window (if you are running on a different platform you will need to edit the file CreateDeployOptimize.bat accordingly), change directory to ```optimization-analyzer/AnalyzeOptimizations``` and run ```CreateDeployOptimize.bat```
